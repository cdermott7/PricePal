...
class ExampleMentraOSApp extends AppServer {
  ...
  private async analyzePhotoWithGemini(photo: StoredPhoto, userId: string) {
    ...
    try {
      ...
      console.log(`[GEMINI] Using model: ${model} for user ${userId}`);
      
      const base64Image = photo.buffer.toString('base64');
      console.log(`[GEMINI] Converted image to base64 for user ${userId}, length: ${base64Image.length}`);
...
2. Provide a one sentence recommendation about whether to buy the product: e.g. if this is a good price and i should buy it, or buy elsewhere or buy an alternative product.`,
      ...
      console.log(`[GEMINI] Configuration prepared for user ${userId}:`, {
        model,
        tools: tools.length,
      ...
      console.log(`[GEMINI] Making API call to Gemini for user ${userId}`);
      ...
      console.log(`[GEMINI] Received response from Gemini for user ${userId}`);
      const analysis = response.text;
      if (!analysis) {
        console.error(`[GEMINI] No analysis text received from Gemini for user ${userId}`);
        return;
      }
      console.log(`[GEMINI] Analysis text length: ${analysis.length} for user ${userId}`);
      console.log(`[GEMINI] Full analysis for user ${userId}:`, analysis);
      ...
      console.error(`[GEMINI] Error analyzing photo with Gemini for user ${userId}:`, error);
